...
class AuthManager {
  ...
  constructor(appName = 'apper', config = {}, logger = console) {
    ...
    const rawAdminPort = config.adminPort !== undefined ?
      config.adminPort :
      (config.admin_port !== undefined ?
        config.admin_port :
        config['admin-port']);
  ...
  setAdminPort(port) {
    this.#adminPort = typeof port === 'number' ? port : Number(port);
  }
    ...
    if (req) {
      ...
      if (host) {
        ...
        const proto =
          ...
          (req.secure ? 'https' : (isLocal ? req.protocol || 'http' : 'https'));
  ...
  getMiddleware() {
    return (req, res, next) => {
      if (!this.#enabled || req.isAdmin) {
        return next();
      }

      if (this.#adminPort !== null && !isNaN(this.#adminPort)) {
        if (req.socket && req.socket.localPort === this.#adminPort) {
          req.isAdmin = true;
          return next();
        }
      }
