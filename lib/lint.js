...
function loadIgnorePatterns(cwd = process.cwd()) {
  const candidateFiles = [
    path.join(cwd, '.apperignore'),
    path.join(cwd, '.eslintignore'),
  ];
  const ignoreFile = candidateFiles.find((f) => fs.existsSync(f));
  if (ignoreFile) {
...
function getConfig(customOverrides = {}, cwd = process.cwd()) {
  const baseRules = Object.assign(getBaseRules(), customOverrides.rules || {});
  const userIgnores = loadIgnorePatterns(cwd);
